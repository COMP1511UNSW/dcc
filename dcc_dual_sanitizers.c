...
static void cleanup_before_exit(void) __attribute__((destructor));
static void cleanup_before_exit(void) {
	debug_printf(3, "cleanup_before_exit\n");
...
static ssize_t __dcc_synchronize_read(void *v, char *buf, size_t size) {
	...
	if (n_bytes_read > 0  && !synchronization_terminated) {
		...
		if (n_bytes_written != n_bytes_read) {
			debug_printf(1, "__dcc_synchronize_read %d != %d\n", (int)n_bytes_written, (int)n_bytes_read);
			synchronization_failed();
		}
	}
#else
	...
	if (n_bytes_read > 0) {
		...
		if (n_bytes_read != n_bytes_actually_read) {
			debug_printf(1, "__dcc_synchronize_read error: read returned %d != %d\n", (int)n_bytes_read, (int)n_bytes_actually_read);
			synchronization_failed();
		}
	}
#endif
...
static int __dcc_synchronize_close(void *v) {
...
FILE *open_cookie(void *cookie, const char *mode) {
	return fopencookie(cookie, mode, (cookie_io_functions_t) {
				.read = __dcc_synchronize_read,
				.write = __dcc_synchronize_write,
				.seek = __dcc_synchronize_seek,
				.close = __dcc_synchronize_close});
}
...
static FILE *get_cookie(FILE *f, const char *mode) {
	if (!f || synchronization_terminated) {
		return f;
	}
